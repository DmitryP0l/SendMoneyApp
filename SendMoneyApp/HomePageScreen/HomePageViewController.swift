...
extension HomePageViewController: UITableViewDelegate, UITableViewDataSource {
	...
	//важное
///не актуально!! -  сделать передачу средств между всеми контактами
///   нужен массив - копия глобального массива, чтобы основной  массив был нужен для первого заполнения, а после того как пойдут транзакции, все записывалось туда. не работает передача потому что массив операций заполняется только в  homePageScreen  из  homePageScreen, операции по переводу, храняться в моделях локально а не глобально.
///   
///   нужна реализация делегата для передачи данных назад
/// возможно данные ддля отображения пр первом запуске беруться их одного места а операции сохраняются в другое
///
/// переход на реалм через протоколы и делегаты
/// создание модели бд
/// изменение глобал логики с учетом новой бд
///
///   добавить кнопку,которая ренерирует и добавляет одного пользователя в базу данных
///
